...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	function addCharacter(char:FlxSprite, name:String) {
		...
		switch(name) {
			...
			case 'boyfriend':
				char.frames = Paths.getSparrowAtlas('tord/dialogue/bf_dialogue');
				char.animation.addByPrefix("normal", "bfDialog", 24, true);
				char.flipX = !char.flipX;
				char.y = 180.45;

			case 'tord':
				char.frames = Paths.getSparrowAtlas('tord/dialogue/tord_dialogue', 'shared');
				char.animation.addByPrefix("normal", "tordDialog", 24);
				char.animation.addByPrefix("angry", "tordPissedDialog", 24);
				char.x -= 436.7;
				char.y = 92.2;

		}
		char.animation.play("normal", true);
	}
